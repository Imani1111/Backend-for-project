...
const addProduct = async (req, res) => {
    ...
    try {
        ...
        if (!name || !description || !price) return res.status(400).json({ success: false, message: 'All fields required' });
        if (isNaN(price)) return res.status(400).json({ success: false, message: 'Price must be a number' });
        ...
        const newProduct = { id: result.insertId, name, description, price, image, category: category || 'General' };
